...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="jangheejung/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'main', url:'https://github.com/8253jang/guestbook.git'
            }
        }
        stage('Build') {
            steps {
    ...
    post { 
        always { 
            slackSend(tokenCredentialId: 'slack-token'
                , channel: '#교육'
                , color: 'good'
                , message: "Heejung-${JOB_NAME} (${BUILD_NUMBER}) 빌드가 끝났습니다. Details: (<${BUILD_URL} | here >)")
        }
        success { 
            slackSend(tokenCredentialId: 'slack-token'
                , channel: '#교육'
